...
update_system() {
    ...
    case $OS_ID in
        ...
        centos|rhel|fedora|rocky|almalinux)
            ...
            ;;
        arch|manjaro)
            pacman -Syu --noconfirm >> "$INSTALL_LOG" 2>&1
            ;;
        alpine)
            apk update >> "$INSTALL_LOG" 2>&1
            apk upgrade >> "$INSTALL_LOG" 2>&1
            ;;
        *)
...
install_dependencies() {
    ...
    case $OS_ID in
        ...
        centos|rhel|fedora|rocky|almalinux)
            if command -v dnf &> /dev/null; then
                dnf install -y $packages >> "$INSTALL_LOG" 2>&1
            else
                yum install -y $packages >> "$INSTALL_LOG" 2>&1
            fi
            ;;
        arch|manjaro)
            pacman -S --noconfirm $packages >> "$INSTALL_LOG" 2>&1
            ;;
        alpine)
            apk add $packages >> "$INSTALL_LOG" 2>&1
            ;;
        *)
...
install_openclaw() {
    ...
    # 询问是否更新系统
    echo -e "${YELLOW}是否更新系统? (推荐) [Y/n]: ${NC}"
    ...
    if [ $install_status -eq 0 ]; then
        ...
        error_msg "OpenClaw 安装失败，请查看日志: $INSTALL_LOG"
    fi
