...
oauth_add_account() {
    ...
    case "$choice" in
        ...
        6)  provider="aws-bedrock" ;;
        7)  provider="azure-openai" ;;
        8)  provider="moonshot" ;;
        9)  provider="minimax" ;;
        10) provider="zhipu-glm" ;;
        11) provider="baidu-wenxin" ;;
        12) provider="aliyun-qwen" ;;
        13) provider="xai-grok" ;;
        14) provider="groq" ;;
        15) provider="deepseek" ;;
        16) provider=$(read_input "请输入提供商名称") ;;
...
tg_approve_pairing() {
    ...
    while true; do
        echo ""
        echo -e "${YELLOW}提示:${NC} 在 Telegram 机器人中发送 ${GREEN}/start${NC} 获取配对码"
        echo ""
