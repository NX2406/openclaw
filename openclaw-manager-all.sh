...
menu_install() {
    ...
    echo ""
    echo -e "${WHITE}${BOLD}步骤 1/3: 检查依赖${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    ...
    echo ""
    echo -e "${WHITE}${BOLD}步骤 2/3: 选择安装方式${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    ...
    echo ""
    echo -e "${WHITE}${BOLD}步骤 3/3: 执行安装${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
...
show_main_menu() {
    ...
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    if command -v openclaw &>/dev/null; then
        local version=$(openclaw --version 2>/dev/null | head -n 1 || echo "未知版本")
        echo -e "  ${GREEN}● OpenClaw 状态:${NC} 已安装 ($version)"
    else
        echo -e "  ${YELLOW}○ OpenClaw 状态:${NC} 未安装"
    fi
    ...
    if [[ -f "$OAUTH_CONFIG" ]] && command -v jq &>/dev/null 2>&1; then
        ...
        local auto_switch=$(jq -r '.auto_switch' "$OAUTH_CONFIG" 2>/dev/null || echo "false")
        if [[ "$auto_switch" == "true" ]]; then
            echo -e "  ${BLUE}○ OAuth 账号:${NC} $account_count 个 (自动切换: 已启用)"
        else
            echo -e "  ${BLUE}○ OAuth 账号:${NC} $account_count 个"
        fi
    fi
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    
    echo -e "${WHITE}${BOLD}  主菜单${NC}\n"
    echo -e "  ${PURPLE}【 安装与更新 】${NC}"
    echo -e "    ${GREEN}1.${NC} 安装/更新 OpenClaw"
    echo ""
    echo -e "  ${PURPLE}【 卸载管理 】${NC}"
    echo -e "    ${GREEN}2.${NC} 扫描已安装组件"
    echo -e "    ${GREEN}3.${NC} 一键完整卸载"
    echo -e "    ${GREEN}4.${NC} 选择性卸载"
    echo ""
    echo -e "  ${PURPLE}【 OAuth 管理 】${NC}"
    echo -e "    ${GREEN}5.${NC} OAuth 账号管理"
    echo ""
    echo -e "  ${PURPLE}【 Telegram 】${NC}"
