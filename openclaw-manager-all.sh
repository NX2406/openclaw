...
# ------------------------------
# 超时工具（避免 openclaw 某些命令卡住）
# - Linux: timeout
# - macOS + coreutils: gtimeout
# ------------------------------
TIMEOUT_BIN=""
...
run_cmd_timeout() {
  # run_cmd_timeout <seconds> <cmd> [args...]
...
' "$_OPENCLAW_VERSION_CACHE"
  ...
  # 某些环境下 openclaw 启动会被网络/更新检查拖慢；这里做超时保护，避免菜单卡住
  local t="${OC_VERSION_TIMEOUT_SEC:-2}"

  # t=0：跳过 version 读取（更快，避免 openclaw 冷启动拖慢菜单）
  if [[ "$t" == "0" ]]; then
    v="已安装"
    rc=0
  elif [[ -n "$TIMEOUT_BIN" ]]; then
    v="$($TIMEOUT_BIN "${t}s" openclaw --version 2>/dev/null)"
    rc=$?
...
pick_agent_id() {
  ...
  # 注意：这个函数经常被用在 $(...) 中；因此把“提示/菜单”输出到 stderr，
  # 只把最终的 agentId 输出到 stdout，避免出现“看起来卡住”的假象。
  {
    echo ""
...
fallback_models_status_local() {
  # 当 openclaw models status 卡住/超时时，做一个“本地只读”的兜底展示
  # 仅解析 auth-profiles.json（不会触发任何网络请求）
  local agent_id="$1"
  print_section "本地兜底：已登录账号（从 auth-profiles.json 读取）"

  local f=""
  if f="$(auth_profiles_file_for_agent "$agent_id" 2>/dev/null)"; then
    :
  else
...
PY
  ...
  echo ""
  echo -e "  ${DIM}提示：如需更完整的“默认模型/回退链/账号状态”，请稍后再试：${NC}"
  echo -e "  ${DIM}  openclaw models status --plain --agent ${agent_id}${NC}"
}
...
models_status_for_agent() {
  ...
  # 说明：models status 默认不会做 live probe（不会发真实请求）；只有加 --probe 才会探测。
  # 这里加 --plain 避免某些终端渲染导致的卡顿，同时加超时保护防止一直挂住。
  local rc=0
  ...
  if [[ $rc -ne 0 ]]; then
    if is_timeout_rc "$rc"; then
      warn "openclaw models status 超时（${OC_MODELS_STATUS_TIMEOUT_SEC}s）。已自动使用本地兜底展示。"
    else
      warn "openclaw models status 执行失败（exit=$rc）。已自动使用本地兜底展示。"
    fi
    echo ""
...
models_check_for_agent() {
  ...
  # --check：exit 1=expired/missing, 2=expiring。加超时避免某些环境卡住。
  local rc=0
...
models_set_default_model() {
  ...
  echo -e "  这个设置只对当前选择的 agent 生效（各 agent 可分别使用不同模型）。"
  echo -e "  输入格式：${GREEN}provider/model${NC}（例如 openai-codex/gpt-5.2）或一个 alias。"
  echo -e "  ${DIM}小技巧：输入 ${BOLD}list${NC}${DIM} 查看模型列表（可能较慢）。回车取消。${NC}"
  echo ""
  ...
  while true; do
    ...
    case "${model,,}" in
      list|ls|\?)
        echo ""
        print_section "models list（仅展示可用模型/别名；不等同于当前默认）"
  ...
  if [[ $rc -eq 0 ]]; then
    ok "已设置默认模型（会话可能有 stickiness；如未生效请 /new 或 /reset 开新会话）"
  else
    err "models set 失败（exit=$rc）"
  fi
  return 0
}
...
codex_auth_order_get() {
  ...
  if is_timeout_rc "$rc"; then
    warn "models auth order get 超时（${OC_AUTH_ORDER_TIMEOUT_SEC}s）。"
  else
    warn "models auth order get 失败（exit=$rc，可能是旧版本 CLI 或配置损坏）。"
  fi

  warn "尝试从配置读取 auth.order[openai-codex] 作为兜底（若你的版本支持 config get 该键）。"
...
codex_auth_order_set() {
  ...
  if [[ $rc -eq 0 ]]; then
    ok "已设置（注意：会话可能有 stickiness；建议 /new 或 /reset 开新会话测试）"
    return 0
  fi

  if is_timeout_rc "$rc"; then
    warn "models auth order set 超时（${OC_AUTH_ORDER_TIMEOUT_SEC}s）。将尝试配置兜底写入。"
  else
    warn "models auth order set 失败（exit=$rc，可能是旧版本 CLI）。将尝试配置兜底写入。"
  fi

  # 兜底：写入 openclaw.json 的 auth.order
  local arr_json
...
codex_auth_order_clear() {
  ...
  if is_timeout_rc "$rc"; then
    warn "models auth order clear 超时（${OC_AUTH_ORDER_TIMEOUT_SEC}s）。将尝试配置兜底清除。"
  else
    warn "models auth order clear 失败（exit=$rc，可能是旧版本 CLI）。将尝试配置兜底清除。"
  fi
