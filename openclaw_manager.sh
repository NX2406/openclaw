...
install_dependencies() {
    ...
    case $OS_ID in
        ...
        # NixOS
        nixos)
            nix-env -iA nixos.curl nixos.wget nixos.git
            ;;
        *)
...
add_oauth_account() {
    ...
    echo ""
    info_msg "正在启动 OAuth 登录流程..."
    echo ""
    echo -e "${YELLOW}请在弹出的浏览器窗口中完成登录${NC}"
    ...
    case $account_type in
        "claude")
            openclaw auth login anthropic --oauth 2>/dev/null || true
            ;;
        "chatgpt")
            openclaw auth login openai --oauth 2>/dev/null || true
            ;;
    esac
...
EOF
    ...
    # 使用 jq 添加账号
    local temp_file=$(mktemp)
    jq ".accounts += [$new_account]" "$ACCOUNTS_CONFIG" > "$temp_file" && mv "$temp_file" "$ACCOUNTS_CONFIG"
    
    echo ""
    success_msg "账号 \"$account_name\" 添加成功!"
    echo ""
    ...
    if [ "$account_count" -gt 1 ]; then
        ...
        if [[ ! "$set_active" =~ ^[Nn]$ ]]; then
            jq ".activeAccountIndex = $((account_count - 1))" "$ACCOUNTS_CONFIG" > "$temp_file" && mv "$temp_file" "$ACCOUNTS_CONFIG"
...
test_accounts() {
    ...
    for i in $(seq 0 $((account_count - 1))); do
        ...
        # 模拟测试 (实际应调用 OpenClaw API 测试)
        sleep 1
        
        # 更新状态为 active (实际应根据测试结果更新)
        local temp_file=$(mktemp)
        jq ".accounts[$i].status = \"active\"" "$ACCOUNTS_CONFIG" > "$temp_file" && mv "$temp_file" "$ACCOUNTS_CONFIG"
