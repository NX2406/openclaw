...
install_openclaw() {
    ...
    info_msg "请按照提示完成安装配置"
    info_msg "看到 'Onboarding complete' 后将自动返回主菜单"
    echo ""
    
    # 下载脚本到临时文件
    local tmp_script="/tmp/openclaw_install_$$.sh"
    local tmp_log="/tmp/openclaw_output_$$.log"
    curl -fsSL https://openclaw.ai/install.sh -o "$tmp_script"
    chmod +x "$tmp_script"
    
    # 使用 script 命令记录输出，同时保持交互功能
    # script 命令会保留完整的终端交互能力
    script -q -c "bash '$tmp_script'" "$tmp_log" &
    local script_pid=$!
    ...
    (
        while kill -0 $script_pid 2>/dev/null; do
            if [ -f "$tmp_log" ]; then
                if grep -qi "Onboarding complete" "$tmp_log" 2>/dev/null; then
                    sleep 5  # 等待用户看到完成信息
                    # 发送 SIGTERM 结束 script 进程
