...
add_oauth_account() {
    ...
    local plugins_ok=false
    if openclaw plugins list 2>&1 | grep -qi "$oauth_provider\|installed"; then
        plugins_ok=true
        echo -e "   ${GREEN}✓${NC} 插件已安装"
    else
        warn_msg "可能缺少必要插件"
        echo ""
        echo -e "${CYAN}是否安装 OpenClaw 插件? [Y/n]: ${NC}"
    ...
    echo -e "${YELLOW}正在获取授权 URL...${NC}"
    echo ""
    echo -e "${WHITE}命令: ${CYAN}openclaw models auth login --provider $oauth_provider --method oauth${NC}"
    echo ""
    
    # 使用 script 命令捕获 PTY 输出
    # script 可以捕获完整的终端输出，包括交互式程序
    echo -e "${GRAY}使用 script 捕获输出...${NC}"
    echo ""
    
    # 设置超时，避免永久等待
    (
        script -q -c "timeout 30 openclaw models auth login --provider $oauth_provider --method oauth" "$output_file" 2>/dev/null || \
        timeout 30 openclaw models auth login --provider "$oauth_provider" --method oauth > "$output_file" 2>&1
    ) &
    local script_pid=$!
    ...
    while [ $wait_time -lt $max_wait ]; do
        ...
        if [ -f "$output_file" ]; then
            auth_url=$(grep -oE 'https://[^[:space:]"<>]+' "$output_file" 2>/dev/null | head -1)
            if [ -n "$auth_url" ]; then
                break
            fi
        fi
        ...
        # 显示等待进度
        echo -ne "\r${GRAY}等待授权 URL... ($wait_time/$max_wait 秒)${NC}"
    done
    echo ""
    ...
    # 检查是否成功获取 URL
    if [ -n "$auth_url" ]; then
        success_msg "已获取授权 URL!"
        echo ""
        echo -e "${WHITE}请在本地浏览器中打开以下链接:${NC}"
        echo ""
        echo -e "${CYAN}${BOLD}$auth_url${NC}"
        echo ""
    else
        # 显示命令输出供用户查看
        warn_msg "未能自动提取授权 URL"
        echo ""
        ...
        echo -e "${GRAY}────────────────────────────────────────${NC}"
        echo ""
        echo -e "${YELLOW}请查看上方输出中的 URL，或手动运行:${NC}"
    ...
    print_line
    echo ""
    echo -e "${WHITE}完成浏览器登录后:${NC}"
    echo ""
    echo -e "${CYAN}请粘贴重定向后的回调 URL (留空跳过): ${NC}"
    ...
    if [ -n "$callback_url" ]; then
        echo ""
        info_msg "正在处理回调 URL..."
        # 某些 OpenClaw 版本可能需要将回调 URL 传递给特定命令
        # 这里我们假设用户已经在原终端中完成了粘贴
        echo -e "${GRAY}回调 URL 已记录${NC}"
    fi
