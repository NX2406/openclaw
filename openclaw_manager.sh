...
add_oauth_account() {
    ...
    echo ""
    echo -e "${YELLOW}提示: 本脚本支持远程服务器 (无头模式)${NC}"
    echo -e "${GRAY}将生成授权链接，您可在本地浏览器完成登录${NC}"
    echo ""
    print_line
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}1.${NC} 在另一个终端窗口运行以下命令:"
    echo ""
    echo -e "      ${CYAN}openclaw auth login $provider --oauth${NC}"
    echo ""
    echo -e "   ${GREEN}2.${NC} 如果提示无法打开浏览器，复制显示的 URL"
    echo ""
    echo -e "   ${GREEN}3.${NC} 在本地浏览器中打开该 URL 并完成登录"
    echo ""
    echo -e "   ${GREEN}4.${NC} 登录成功后，将重定向的 URL 粘贴回终端"
    echo ""
    echo -e "   ${GREEN}5.${NC} 完成后返回此窗口"
    echo ""
    print_line
    echo ""
    echo -e "${YELLOW}是否已完成上述 OAuth 登录流程? [y/N]: ${NC}"
    read -r oauth_done </dev/tty
    ...
    # 方法3: 尝试运行 openclaw 状态检查
    if [ "$verification_passed" = false ]; then
        if timeout 15 openclaw status 2>/dev/null | grep -qi "authenticated\|connected\|ready"; then
            account_status="active"
            verification_passed=true
            echo -e "   ${GREEN}✓${NC} OpenClaw 状态检查通过"
        fi
    fi
