...
add_oauth_account() {
    ...
    # 检查并安装必要的插件
    info_msg "检查 OpenClaw 插件..."
    echo ""
    
    # 检测是否有可用插件
    local plugin_check=$(openclaw plugins list 2>&1)
    
    if echo "$plugin_check" | grep -qi "no.*plugin\|not found\|error\|empty"; then
        warn_msg "未安装任何插件，需要先安装！"
        echo ""
        echo -e "${WHITE}运行: ${CYAN}openclaw plugins install${NC}"
        echo ""
        echo -e "${CYAN}是否现在安装插件? [Y/n]: ${NC}"
        ...
        if [[ ! "$install_plugins" =~ ^[Nn]$ ]]; then
            ...
            if [ $install_result -eq 0 ]; then
                ...
                warn_msg "插件安装返回状态码: $install_result"
            fi
        else
            error_msg "需要先安装插件才能进行 OAuth 认证"
            echo ""
            echo -e "${YELLOW}请手动运行: ${CYAN}openclaw plugins install${NC}"
            press_any_key
            return
        fi
    else
        ...
        echo ""
        echo -e "${GRAY}已安装的插件:${NC}"
        echo "$plugin_check" | head -10
    fi
    ...
    echo -e "${YELLOW}${BOLD}正在启动 OAuth 认证...${NC}"
    echo ""
    echo -e "${WHITE}即将运行命令:${NC}"
    echo -e "${CYAN}openclaw models auth login --provider $oauth_provider --method oauth${NC}"
    echo ""
    print_line
    echo ""
    echo -e "${WHITE}${BOLD}操作指南:${NC}"
    echo ""
    echo -e "   ${GREEN}1.${NC} 命令会显示一个 ${CYAN}授权 URL${NC}"
    echo -e "   ${GREEN}2.${NC} ${YELLOW}复制这个 URL${NC} 到您的本地浏览器打开"
    echo -e "   ${GREEN}3.${NC} 在浏览器中完成登录"
    echo -e "   ${GREEN}4.${NC} 浏览器会跳转到一个新 URL (可能显示错误页面，这是正常的)"
    echo -e "   ${GREEN}5.${NC} ${YELLOW}复制浏览器地址栏的完整 URL${NC}"
    echo -e "   ${GREEN}6.${NC} ${YELLOW}粘贴回下方的终端${NC} 完成认证"
    echo ""
    print_line
    echo ""
    echo -e "${CYAN}按任意键开始 OAuth 认证...${NC}"
    read -n 1 -s -r </dev/tty
    echo ""
    
    # 直接在前台运行命令 - 用户可以看到所有输出并交互
    ...
    if [ $auth_result -eq 0 ]; then
        success_msg "OAuth 认证流程完成!"
    else
        warn_msg "OAuth 认证返回状态码: $auth_result"
        echo ""
        echo -e "${YELLOW}如果认证未成功，您可以稍后手动运行:${NC}"
        echo -e "${CYAN}openclaw models auth login --provider $oauth_provider --method oauth${NC}"
    fi
