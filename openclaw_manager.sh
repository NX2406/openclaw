...
install_openclaw() {
    ...
    info_msg "安装完成后将自动返回主菜单..."
    echo ""
    
    # 下载脚本到临时文件
    local tmp_script="/tmp/openclaw_install_$$.sh"
    local tmp_log="/tmp/openclaw_install_output_$$.log"
    ...
    # 等待安装完成（检测 "Onboarding complete" 或 "installed" 等关键字）
    local timeout_count=0
    local max_timeout=600  # 最长等待10分钟
    ...
    while [ $timeout_count -lt $max_timeout ]; do
        ...
        # 检查日志中是否有完成标志
        if [ -f "$tmp_log" ]; then
            if grep -qi "Onboarding complete\|Installation complete\|openclaw installed\|✓ OpenClaw installed" "$tmp_log" 2>/dev/null; then
                sleep 3  # 等待几秒让输出完成
                install_complete=true
                # 不要杀死进程！让 OpenClaw 服务继续在后台运行
                # 使用 disown 将进程从当前 shell 分离
                disown $script_pid 2>/dev/null
                break
            fi
        fi
    ...
    # 检查是否真的安装成功
    if command -v openclaw &> /dev/null || [ "$install_complete" = true ]; then
        echo ""
