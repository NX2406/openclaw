...
add_oauth_account() {
    ...
    # 显示支持的 provider
    info_msg "支持的 OAuth Provider:"
    echo ""
    echo -e "   • ${GREEN}openai-codex${NC} - ChatGPT Plus/Pro 订阅"
    echo -e "   • ${CYAN}google-antigravity${NC} - Gemini Pro 订阅"
    echo -e "   • ${PURPLE}anthropic${NC} - Claude Pro/Max 订阅"
    echo ""
    
    # 根据账号类型确定 provider
    local oauth_provider
    case $account_type in
        "claude") oauth_provider="anthropic" ;;
        "chatgpt") oauth_provider="openai-codex" ;;
        *) oauth_provider="$provider" ;;
    esac
    
    echo -e "${WHITE}将使用 Provider: ${CYAN}$oauth_provider${NC}"
    echo ""
    print_line
    echo ""
    
    # 首先检查并安装必要的插件
    info_msg "检查 OpenClaw 插件状态..."
    echo ""
    
    # 尝试获取插件列表
    local plugin_output=$(openclaw plugins list 2>&1)
    local need_install=false
    
    # 显示当前插件状态
    echo -e "${GRAY}$plugin_output${NC}"
    echo ""
    
    # 检测各种"没有插件"的情况
    if echo "$plugin_output" | grep -qiE "no.*plugin|not found|error|empty|没有|未安装"; then
        need_install=true
    fi
    ...
    if [ "$need_install" = true ]; then
        echo ""
        warn_msg "需要安装 OpenClaw 插件才能使用 OAuth 认证"
        echo ""
        echo -e "${WHITE}请运行以下命令安装插件:${NC}"
        echo ""
        echo -e "   ${CYAN}${BOLD}openclaw plugins install${NC}"
        echo ""
        echo -e "${CYAN}是否现在安装? [Y/n]: ${NC}"
        ...
        if [[ ! "$do_install" =~ ^[Nn]$ ]]; then
            ...
            print_line
            echo ""
            info_msg "正在安装 OpenClaw 插件..."
            echo -e "${GRAY}请等待安装完成...${NC}"
            echo ""
            ...
            # 再次检查是否安装成功
            local verify_output=$(openclaw plugins list 2>&1)
            if echo "$verify_output" | grep -qiE "no.*plugin|not found|error"; then
                error_msg "插件安装可能失败，请手动安装"
                echo ""
                echo -e "${YELLOW}手动运行: ${CYAN}openclaw plugins install${NC}"
                press_any_key
                return
            else
                success_msg "插件安装成功!"
            ...
            warn_msg "跳过插件安装，OAuth 认证可能会失败"
        fi
    else
        echo -e "   ${GREEN}✓${NC} 插件已就绪"
    fi
    ...
    echo -e "${YELLOW}${BOLD}开始 OAuth 认证${NC}"
    echo ""
    echo -e "${WHITE}即将运行:${NC}"
    echo -e "${CYAN}openclaw models auth login --provider $oauth_provider --method oauth${NC}"
    echo ""
    print_line
    echo ""
    echo -e "${WHITE}${BOLD}操作步骤:${NC}"
    echo ""
    echo -e "   ${GREEN}1.${NC} 命令会显示一个 ${CYAN}授权 URL${NC}"
    echo -e "   ${GREEN}2.${NC} ${YELLOW}复制 URL${NC} 到本地浏览器打开"
    echo -e "   ${GREEN}3.${NC} 完成登录后复制地址栏的 ${YELLOW}回调 URL${NC}"
    echo -e "   ${GREEN}4.${NC} ${YELLOW}粘贴回这里${NC} 完成认证"
    echo ""
    print_line
    echo ""
    ...
    # 直接运行 OAuth 认证 (前台交互)
    openclaw models auth login --provider "$oauth_provider" --method oauth </dev/tty
    ...
    if [ $auth_result -eq 0 ]; then
        success_msg "OAuth 认证完成!"
    else
        warn_msg "OAuth 返回状态码: $auth_result"
        echo ""
        
        # 检查是否是插件问题
        if [ $auth_result -eq 1 ]; then
            echo -e "${YELLOW}如果看到 'No provider plugins found' 错误:${NC}"
            echo -e "   请先运行: ${CYAN}openclaw plugins install${NC}"
            echo ""
        fi
        
        echo -e "${GRAY}手动重试: openclaw models auth login --provider $oauth_provider --method oauth${NC}"
    fi
    ...
    # 检查 auth-profiles.json (正确的路径)
    local auth_profile="$HOME/.openclaw/agents/main/agent/auth-profiles.json"
    if [ -f "$auth_profile" ]; then
        if jq -e ".$oauth_provider" "$auth_profile" >/dev/null 2>&1; then
            account_status="active"
            verification_passed=true
            echo -e "   ${GREEN}✓${NC} 在 auth-profiles.json 中发现 $oauth_provider 凭证"
        fi
    fi
    ...
    if [ "$verification_passed" = false ]; then
        for profile_file in "$HOME/.openclaw"/agents/*/agent/auth-profiles.json; do
            if [ -f "$profile_file" ]; then
                if jq -e ".$oauth_provider" "$profile_file" >/dev/null 2>&1; then
                    account_status="active"
                    verification_passed=true
                    echo -e "   ${GREEN}✓${NC} 在 $(basename $(dirname $(dirname $profile_file)))/auth-profiles.json 中发现凭证"
                    break
                fi
            fi
        done
