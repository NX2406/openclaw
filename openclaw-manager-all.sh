...
oauth_list_accounts() {
    ...
    local config_paths=(
        ...
        "$OAUTH_CONFIG"
        "${HOME}/.openclaw/credentials/oauth.json"
    ...
    if [[ -z "$found_config" ]]; then
        ...
        echo ""
        echo -e "  ${DIM}已检查以下路径:${NC}"
        for path in "${config_paths[@]}"; do
            if [[ -e "$path" ]]; then
                echo -e "    ${YELLOW}✗ $path${NC} ${DIM}(存在但可能为空或格式不对)${NC}"
            else
                echo -e "    ${DIM}✗ $path (不存在)${NC}"
            fi
        done
        echo ""
        echo -e "  ${YELLOW}${BOLD}💡 故障排查:${NC}"
        echo ""
        echo -e "  ${WHITE}1. 检查配置目录${NC}"
        echo -e "     ${DIM}运行: ${GREEN}ls -la ~/.openclaw/config/${NC}"
        echo ""
        echo -e "  ${WHITE}2.  查看完整配置${NC}"
        echo -e "     ${DIM}运行: ${GREEN}cat ~/.openclaw/config/openclaw.json${NC}"
        echo ""
        echo -e "  ${WHITE}3. 重新运行配置向导${NC}"
        echo -e "     ${DIM}运行: ${GREEN}openclaw onboard${NC}"
        echo ""
        press_any_key
        return 1
    fi
    ...
    if [[ "$count" == "0" ]] || [[ "$count" == "null" ]]; then
        echo ""
        print_warning "配置文件中没有 accounts 字段或为空"
        echo ""
        echo -e "  ${DIM}配置文件: $found_config${NC}"
        echo ""
        echo -e "  ${YELLOW}💡 显示完整配置结构:${NC}"
        echo ""
        
        if jq '.' "$found_config" 2>/dev/null; then
            # jq 成功解析
            echo ""
        else
            # jq 失败，显示原始内容
        ...
        echo -e "  ${YELLOW}💡 提示:${NC}"
        echo -e "    OpenClaw 配置文件可能使用不同的结构"
        echo -e "    如果看到了提供商信息，说明账号已配置成功"
        echo ""
        press_any_key
        return 0
    fi
...
oauth_add_account() {
    ...
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BLUE}${BOLD}  OpenClaw OAuth 配置${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    
    echo -e "  ${WHITE}${BOLD}📋 配置方法${NC}"
    echo ""
    echo -e "  ${GREEN}方法 1: 使用配置向导 (推荐)${NC}"
    echo -e "    ${DIM}运行: ${GREEN}openclaw onboard${NC}"
    echo -e "    ${DIM}按照向导提示选择提供商并授权${NC}"
    echo ""
    echo -e "  ${GREEN}方法 2: 手动配置${NC}"
    echo -e "    ${DIM}编辑: ${GREEN}~/.openclaw/config/openclaw.json${NC}"
    echo -e "    ${DIM}添加提供商配置和凭据${NC}"
    echo ""
    
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    
    echo -e "${YELLOW}${BOLD}支持的主流提供商:${NC}"
    echo ""
    echo -e "  ${WHITE}•${NC} Google Gemini"
    echo -e "  ${WHITE}•${NC} Claude (Anthropic)"
    echo -e "  ${WHITE}•${NC} OpenAI (ChatGPT)"
    echo -e "  ${WHITE}•${NC} OpenRouter"
    echo -e "  ${WHITE}•${NC} GitHub Copilot"
    echo -e "  ${WHITE}•${NC} 其他（Azure, Bedrock, 国内提供商等）"
    echo ""
    
    if confirm_action "是否现在运行配置向导?"; then
        echo ""
        print_info "正在启动 OpenClaw 配置向导..."
        echo ""
        echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo ""
        ...
        if [[ $exit_code -eq 0 ]]; then
            ...
            print_warning "配置过程已退出"
            echo ""
            echo -e "  ${DIM}你可以稍后运行 ${GREEN}openclaw onboard${DIM} 继续配置${NC}"
        fi
    else
        print_info "你可以稍后手动运行配置"
        echo ""
        echo -e "  ${WHITE}运行命令:${NC}"
        echo -e "    ${GREEN}openclaw onboard${NC}"
        echo ""
        echo -e "  ${WHITE}或编辑配置文件:${NC}"
        echo -e "    ${GREEN}~/.openclaw/config/openclaw.json${NC}"
    fi
