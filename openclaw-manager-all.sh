...
menu_install() {
    ...
    if command -v openclaw &>/dev/null; then
        ...
        echo ""
        echo -e "  ${YELLOW}${BOLD}💡 建议的后续步骤:${NC}"
        echo -e "  ${DIM}   → 返回主菜单选择 ${GREEN}[5]${DIM} 添加 OAuth 账号${NC}"
        echo -e "  ${DIM}   → 然后选择 ${GREEN}[6]${DIM} 配对 Telegram 机器人${NC}"
    else
...
oauth_list_accounts() {
    ...
    # 优先从 OpenClaw CLI 读取账号
    if command -v openclaw &>/dev/null; then
        print_info "正在从 OpenClaw CLI 读取账号信息..."
        echo ""
        
        # 尝试使用 openclaw auth list 命令
        if openclaw auth list 2>/dev/null | grep -q .; then
            echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            echo -e "${BLUE}  OpenClaw OAuth 账号${NC}"
            echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            echo ""
            openclaw auth list 2>/dev/null
            echo ""
            echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            echo ""
            press_any_key
        ...
        # 尝试使用 openclaw accounts 命令
        if openclaw accounts 2>/dev/null | grep -q .; then
            echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            echo -e "${BLUE}  OpenClaw OAuth 账号${NC}"
            echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            echo ""
            openclaw accounts 2>/dev/null
            echo ""
            echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
            echo ""
            press_any_key
    ...
    # 回退到 JSON 配置文件方式
    if ! check_jq; then
        print_warning "无法读取账号信息（需要 jq 工具）"
        press_any_key
        return 1
    fi
    ...
    local config_paths=(
        ...
        "${HOME}/.config/openclaw/auth.json"
        "${HOME}/.openclaw/auth.json"
    )
    ...
    if [[ -z "$found_config" ]]; then
        ...
        echo ""
        echo -e "  ${YELLOW}💡 建议:${NC}"
        echo -e "    1. 使用 OpenClaw CLI 添加账号: ${GREEN}openclaw auth add${NC}"
        echo -e "    2. 或在本脚本中选择 '2. 添加新账号'"
        echo ""
        press_any_key
        return 1
    fi
    
    local count=$(jq '.accounts | length' "$found_config" 2>/dev/null || echo "0")
    
    if [[ "$count" == "0" ]] || [[ "$count" == "null" ]]; then
        echo ""
        print_warning "配置文件中没有账号"
        echo ""
        echo -e "  ${DIM}配置文件: $found_config${NC}"
    ...
    # 账号列表
    jq -r '.accounts[] | "\(.id)|\(.provider)|\(.email)|\(if .is_active then "✓ 活动" else "  未活动" end)"' "$found_config" 2>/dev/null | while IFS='|' read -r id provider email status; do
        if [[ "$status" == *"活动"* ]]; then
    ...
    # 显示自动切换状态
    local auto_switch=$(jq -r '.auto_switch' "$found_config" 2>/dev/null || echo "false")
...
oauth_add_account() {
    ...
    if ! command -v openclaw &>/dev/null; then
        ...
        print_info "请先安装 OpenClaw，然后再添加账号"
        press_any_key
        return 1
    fi
    ...
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BLUE}${BOLD}  选择 OAuth 提供商${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    echo -e "  ${WHITE}【主流提供商】${NC}"
    echo -e "    ${GREEN}1.${NC}  Google Gemini          ${DIM}(推荐)${NC}"
    echo -e "    ${GREEN}2.${NC}  Claude (Anthropic)     ${DIM}(Pro/Max)${NC}"
    echo -e "    ${GREEN}3.${NC}  OpenAI (ChatGPT)       ${DIM}(Plus/Team)${NC}"
    echo ""
    echo -e "  ${WHITE}【API 平台】${NC}"
    echo -e "    ${GREEN}4.${NC}  OpenRouter             ${DIM}(多模型聚合)${NC}"
    echo -e "    ${GREEN}5.${NC}  GitHub Copilot"
    echo ""
    echo -e "  ${WHITE}【云服务】${NC}"
    echo -e "    ${GREEN}6.${NC}  Amazon Bedrock"
    echo -e "    ${GREEN}7.${NC}  Microsoft Azure"
    echo ""
    echo -e "  ${WHITE}【国内提供商】${NC}"
    echo -e "    ${GREEN}8.${NC}  Moonshot AI (Kimi)"
    echo -e "    ${GREEN}9.${NC}  MiniMax"
    echo -e "    ${GREEN}10.${NC} 智谱 AI (GLM)"
    echo -e "    ${GREEN}11.${NC} 百度 (文心一言)"
    echo -e "    ${GREEN}12.${NC} 阿里云 (通义千问)"
    echo ""
    echo -e "  ${WHITE}【其他】${NC}"
    echo -e "    ${GREEN}13.${NC} xAI (Grok)"
    echo -e "    ${GREEN}14.${NC} Groq                   ${DIM}(快速推理)${NC}"
    echo -e "    ${GREEN}15.${NC} DeepSeek"
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    # 映射到 OpenClaw 官方提供商名称
    case "$choice" in
        1)  provider="google" ;;        # Google Gemini
        2)  provider="anthropic" ;;     # Claude
        3)  provider="openai" ;;        # OpenAI/ChatGPT
        4)  provider="openrouter" ;;    # OpenRouter
        5)  provider="github" ;;        # GitHub Copilot
        6)  provider="bedrock" ;;       # AWS Bedrock
        7)  provider="azure" ;;         # Microsoft Azure
        8)  provider="moonshot" ;;      # Moonshot AI
        9)  provider="minimax" ;;       # MiniMax
        10) provider="zhipu" ;;         # 智谱 AI
        11) provider="baidu" ;;         # 百度
        12) provider="alibaba" ;;       # 阿里云
        13) provider="xai" ;;           # xAI
        14) provider="groq" ;;          # Groq
        15) provider="deepseek" ;;      # DeepSeek
    ...
    # 显示操作说明
    echo -e "${YELLOW}${BOLD}📋 接下来的操作:${NC}"
    echo ""
    echo -e "  ${WHITE}1.${NC} OpenClaw 会打开浏览器进行 OAuth 认证"
    echo -e "  ${WHITE}2.${NC} 请在浏览器中登录并授权"
    echo -e "  ${WHITE}3.${NC} 授权成功后会自动添加到 OpenClaw"
    echo ""
    
    if ! confirm_action "是否继续添加账号?"; then
        print_info "已取消"
        press_any_key
        return
    fi
    ...
    auth_output=$(openclaw auth add --provider "$provider" 2>&1)
    auth_exit_code=$?
    ...
    if [[ $auth_exit_code -eq 0 ]]; then
        print_success "✅ OAuth 账号添加成功!"
        echo ""
        echo -e "  ${WHITE}账号已添加到 OpenClaw 配置${NC}"
        echo -e "  ${DIM}OpenClaw 会自动管理账号切换和速率限制${NC}"
    else
        print_error "❌ 账号添加失败"
        echo ""
        
        # 分析失败原因
        if echo "$auth_output" | grep -iq "already exists"; then
            echo -e "  ${YELLOW}原因: 该账号已存在${NC}"
            echo -e "  ${DIM}可以在账号列表中查看已有账号${NC}"
        elif echo "$auth_output" | grep -iq "invalid provider"; then
            echo -e "  ${YELLOW}原因: 不支持的提供商${NC}"
            echo -e "  ${DIM}请检查 OpenClaw 版本是否支持该提供商${NC}"
        elif echo "$auth_output" | grep -iq "authentication failed"; then
            echo -e "  ${YELLOW}原因: 认证失败${NC}"
            echo -e "  ${DIM}请确认:${NC}"
            echo -e "    ${DIM}• 浏览器中已成功登录${NC}"
            echo -e "    ${DIM}• 已授权 OpenClaw 访问权限${NC}"
            echo -e "    ${DIM}• 账号有效且未被封禁${NC}"
        else
            echo -e "  ${DIM}详细错误:${NC}"
            echo "$auth_output" | head -10
        fi
    fi
...
show_oauth_auto_info() {
    print_header
    print_section "账号切换与速率限制管理"
    
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BLUE}${BOLD}  OpenClaw 内置账号管理${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    
    echo -e "  ${WHITE}${BOLD}✨ 自动账号轮换${NC}"
    echo -e "  ${DIM}OpenClaw 内置智能账号切换机制，无需手动配置${NC}"
    echo ""
    
    echo -e "  ${GREEN}●${NC} ${WHITE}工作原理:${NC}"
    echo -e "    ${DIM}1. 当某个账号达到速率限制时${NC}"
    echo -e "    ${DIM}2. OpenClaw 自动切换到下一个可用账号${NC}"
    echo -e "    ${DIM}3. 实现无缝连续服务${NC}"
    echo ""
    
    echo -e "  ${GREEN}●${NC} ${WHITE}配置方式:${NC}"
    echo -e "    ${DIM}只需添加多个 OAuth 账号即可${NC}"
    echo -e "    ${DIM}OpenClaw 会自动管理切换逻辑${NC}"
    echo ""
    
    echo -e "  ${GREEN}●${NC} ${WHITE}查看配置:${NC}"
    echo -e "    ${DIM}配置文件: ${GREEN}~/.openclaw/config/openclaw.json${NC}"
    echo -e "    ${DIM}查看已添加账号: 选择菜单中的 '查看所有账号'${NC}"
    echo ""
    
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    
    echo ""
    echo -e "${YELLOW}${BOLD}💡 推荐配置:${NC}"
    echo ""
    echo -e "  ${WHITE}•${NC} ${DIM}添加2-3个相同类型的账号（如多个 Google 账号）${NC}"
    echo -e "  ${WHITE}•${NC} ${DIM}或添加不同提供商账号作为 fallback（如 Google + Claude）${NC}"
    echo -e "  ${WHITE}•${NC} ${DIM}OpenClaw 会智能分配请求，避免单点速率限制${NC}"
    echo ""
...
menu_oauth() {
    while true; do
        ...
        # 从 OpenClaw CLI 获取账号信息
        if command -v openclaw &>/dev/null; then
            local account_count=$(openclaw auth list 2>/dev/null | grep -c "^" || echo "0")
            if [[ "$account_count" -gt 0 ]]; then
                echo -e "  ${BLUE}📊 已添加账号:${NC} ${GREEN}$account_count${NC} 个"
                echo -e "  ${BLUE}🔄 自动轮换:${NC} ${GREEN}已启用${NC} ${DIM}(OpenClaw 内置)${NC}"
            else
                echo -e "  ${YELLOW}⚠ 暂无已添加账号${NC}"
            fi
        else
        ...
        echo -e "    ${GREEN}3.${NC} 🗑️  删除账号"
        echo -e "    ${GREEN}4.${NC} ℹ️  账号轮换说明"
        ...
        case "$choice" in
            ...
            2) oauth_add_account ;;
            3) oauth_delete_account ;;
            4) show_oauth_auto_info ;;
            0) return ;;
...
tg_check_openclaw() {
    ...
    # 检查 OpenClaw 服务是否正在运行
    if ! pgrep -f "openclaw" &>/dev/null; then
        print_warning "OpenClaw 服务未运行"
        echo ""
        echo -e "  ${YELLOW}配对需要 OpenClaw 服务运行中${NC}"
        echo ""
        echo -e "  ${WHITE}启动服务:${NC}"
        echo -e "    ${DIM}• 手动启动: ${GREEN}openclaw${NC}"
        echo -e "    ${DIM}• 或使用 PM2: ${GREEN}pm2 start openclaw${NC}"
        echo -e "    ${DIM}• 或使用 Docker: ${GREEN}docker compose up -d${NC}"
        echo ""
        
        if confirm_action "是否现在启动 OpenClaw 服务?"; then
            echo ""
            print_info "正在启动 OpenClaw..."
            ...
            if command -v nohup &>/dev/null; then
                ...
                if pgrep -f "openclaw" &>/dev/null; then
                    print_success "OpenClaw 服务已启动"
                    echo ""
                    return 0
                else
                    print_error "自动启动失败，请手动启动 OpenClaw"
                    press_any_key
                    return 1
                fi
            else
                print_warning "请在新终端中运行: openclaw"
                press_any_key
                return 1
            fi
        else
...
tg_approve_pairing() {
    ...
    while true; do
        echo ""
        echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo -e "${YELLOW}${BOLD}📱 如何获取配对码${NC}"
        echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo -e "  ${WHITE}1.${NC} 打开 Telegram，找到你的 OpenClaw 机器人"
        echo -e "  ${WHITE}2.${NC} 发送 ${GREEN}/start${NC} 命令"
        echo -e "  ${WHITE}3.${NC} 机器人会回复配对码（例如: ${YELLOW}SGRJEKL7${NC}）"
        echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo ""
        ...
        # 步骤3: 配对结果
        local pairing_output=$(openclaw pairing approve telegram "$code" 2>&1)
        local pairing_exit_code=$?
        ...
        if [[ $pairing_exit_code -eq 0 ]]; then
            ...
            # 分析失败原因
            if echo "$pairing_output" | grep -iq "no pending pairing request"; then
                echo -e "  ${RED}${BOLD}原因:${NC} ${YELLOW}OpenClaw 未收到配对请求${NC}"
                echo ""
                echo -e "  ${WHITE}请确认:${NC}"
                echo -e "    ${DIM}✓ 已在 Telegram 机器人中发送 ${GREEN}/start${DIM} 命令${NC}"
                echo -e "    ${DIM}✓ 配对码正确（注意大小写）${NC}"
                echo -e "    ${DIM}✓ OpenClaw 服务正在运行${NC}"
                echo -e "    ${DIM}✓ 配对码未过期（若已过期请重新发送 /start）${NC}"
            elif echo "$pairing_output" | grep -iq "already paired"; then
                echo -e "  ${YELLOW}此设备已配对${NC}"
            else
                echo -e "  ${DIM}错误详情: $pairing_output${NC}"
            fi
            
            echo ""
            if confirm_action "是否重新输入配对码?"; then
                print_header
                print_section "批准 Telegram 配对"
                continue
            else
...
show_main_menu() {
    ...
    echo -e "  ${GREEN}2.${NC} 扫描已安装组件"
    echo -e "  ${GREEN}3.${NC} 一键完整卸载"
    echo -e "  ${GREEN}4.${NC} 选择性卸载"
    echo -e "  ${GREEN}5.${NC} OAuth 账号管理"
