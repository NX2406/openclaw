...
tg_approve_pairing() {
    ...
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BLUE}${BOLD}  如何获取配对码${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "  ${WHITE}步骤 1:${NC} 打开 Telegram，找到你的 OpenClaw 机器人"
    echo -e "  ${WHITE}步骤 2:${NC} 向机器人发送 ${GREEN}/start${NC} 命令"
    echo -e "  ${WHITE}步骤 3:${NC} 机器人会回复包含 ${YELLOW}Pairing code${NC} 的消息"
    echo -e "  ${DIM}         例如: Pairing code: SGRJEKL7${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    
    # 显示待批准列表
    echo -e "${WHITE}当前待批准的配对请求:${NC}"
    echo ""
    if openclaw pairing list telegram 2>/dev/null | grep -q .; then
        openclaw pairing list telegram 2>/dev/null
        echo ""
    else
        echo -e "  ${YELLOW}⚠ 暂无待批准请求${NC}"
        echo ""
    fi
    
    code=$(read_input "请输入配对码 (7位字符)")
    ...
    # 验证配对码格式（通常是7位字母数字组合）
    if [[ ! "$code" =~ ^[A-Z0-9]{5,10}$ ]]; then
        print_warning "配对码格式可能不正确（通常为大写字母+数字组合）"
    ...
    if openclaw pairing approve telegram "$code" 2>&1; then
        ...
        echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        echo ""
        echo -e "  ${WHITE}现在你可以通过 Telegram 机器人控制 OpenClaw 了${NC}"
    else
        echo ""
        echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        print_error "❌ 配对失败"
        ...
        echo -e "    • 配对码输入错误"
        echo -e "    • 配对码已过期（需要重新在 TG 发送 /start）"
        echo -e "    • OpenClaw 服务未运行"
    fi
