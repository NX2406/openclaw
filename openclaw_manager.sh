...
add_oauth_account() {
    ...
    # 直接运行 OAuth 登录命令
    echo -e "${YELLOW}正在启动 OAuth 认证...${NC}"
    echo ""
    echo -e "${WHITE}命令: ${CYAN}openclaw models auth login --provider $oauth_provider --method oauth${NC}"
    echo ""
    echo -e "${GRAY}请按照提示完成 OAuth 认证:${NC}"
    echo -e "${GRAY}1. 复制显示的授权 URL${NC}"
    echo -e "${GRAY}2. 在本地浏览器中打开并登录${NC}"
    echo -e "${GRAY}3. 将重定向的 URL 粘贴回终端${NC}"
    echo ""
    print_line
    echo ""
    
    # 运行 OAuth 登录
    openclaw models auth login --provider "$oauth_provider" --method oauth
    ...
    if [ $auth_result -eq 0 ]; then
        success_msg "OAuth 认证命令执行完成"
    else
        warn_msg "OAuth 认证命令返回非零状态: $auth_result"
    fi
